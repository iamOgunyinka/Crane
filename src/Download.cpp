    ...
    void DownloadComponent::startDownloadImpl( int threads_to_use )
    {
        ...
        for( int i = 1; i <= threads_to_use; ++i )
        {
            ...
            if( thread_list_empty ){ // starting download afresh
                ...
                if( i == threads_to_use ){
                    int high_byte = download_information->size_of_file_in_bytes == 0 ? 1 : download_information->size_of_file_in_bytes;
                    download_information->thread_information_list.push_back( Information::ThreadInfo{
                        byte_begin, high_byte, byte_begin, 0, i } );
                    download = new DownloadItem( QUrl( download_information->redirected_url ), byte_begin,
                            high_byte, byte_begin, new_thread );
                } else {
                    byte_end += increment;
